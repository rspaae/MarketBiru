...
function AdminScanContent() {
  ...
  // Extract clean order code from raw QR text, URL, or query param
  // Supports: "MB11-xxxxxx", URL contains "?code=MB11-...", full /admin/scan?code= URLs
  const extractOrderCode = (rawText: string): string => {
    const clean = rawText.trim();
    // Priority 1: URL with ?code= query param (generated by our QR)
    try {
      const url = new URL(clean);
      const codeParam = url.searchParams.get('code');
      if (codeParam) return codeParam.trim().toUpperCase();
    } catch (_) {}
    // Priority 2: Any URL path segment matching MB11- or MKT- pattern
    const matchCode = clean.match(/([A-Z0-9]+-[A-Z0-9-]+)/i);
    if (matchCode) return matchCode[0].toUpperCase();
    return clean.toUpperCase();
  };

  const handleLookup = async (rawText: string) => {
    setErrorMessage('');
    setSuccessMessage('');
    ...
    setIsLookingUp(true);
    try {
      // Cari pesanan dari database MySQL (real-time, cross-device)
  ...
  const handleCompleteOrder = async (orderId: string) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setErrorMessage('Gagal menyelesaikan pesanan: ' + (data.error || 'Unknown error'));
      }
    } catch (err) {
  ...
  // Auto-lookup when page opens with ?code= query param (from QR redirect / scan)
  useEffect(() => {
    ...
    if (codeParam) {
      ...
      setManualCode(upperCode);
      setScanMode('manual');
      handleLookup(upperCode);
    }
          ...
          <div className="bg-white rounded-3xl border border-slate-200/80 p-6 shadow-xs space-y-4">
            ...
            >
              <input
                ...
                placeholder="Contoh: MB11-17258XXXX"
              ...
              <button
                ...
                className="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 disabled:opacity-60 disabled:cursor-wait text-white rounded-xl text-xs font-bold shadow-xs transition flex items-center gap-1.5"
              >
                {isLookingUp ? (
              ...
              <div className="space-y-2">
                {scannedOrder.status !== 'completed' ? (
                  <button
                    ...
                    className="w-full py-4 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 disabled:opacity-60 disabled:cursor-wait text-white rounded-2xl font-black text-sm shadow-lg shadow-emerald-500/20 flex items-center justify-center gap-2 transition active:scale-95"
                  >
                    {isCompleting ? (
                      <><RefreshCw className="w-5 h-5 animate-spin" /> Memproses...</>
                    ) : (
                      <><CheckCircle2 className="w-5 h-5" /><span>Verifikasi &amp; Serahkan Barang ke Siswa</span></>
                    )}
                  </button>
                ) : (
            ...
            <div className="bg-white rounded-3xl border border-dashed border-slate-300 p-12 text-center space-y-4">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-slate-400 max-w-sm mx-auto">
                  Arahkan kamera ke QR nota siswa atau input kode nota secara manual. Data pesanan diambil langsung dari database.
                </p>
              </div>
            </div>
          )}
    ...
    <Suspense
      fallback={
        <div className="p-12 text-center text-slate-400">
          ...
          <p className="text-xs font-semibold">Memuat scanner...</p>
        </div>
      }
    >
      <AdminScanContent />
